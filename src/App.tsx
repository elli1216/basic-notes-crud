...
function App() {
    ...
    return (
      <ul className='grid grid-flow-dense grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-2 p-2'>
        {data.map((note) => (
          ...
          key={note.id}
        >
          <h2 className='text-center p-2 text-3xl font-bold w-fit'>{note.title}</h2>
          <p className='text-2xl'>{note.content}</p>
          <div className='note-actions flex justify-center gap-4 my-3'>
            <img className='hover:cursor-pointer' src={PenToSquare} alt="Edit" width={'30rem'} onClick={() => handleNoteClick(note)}/>
            <img className='hover:cursor-pointer' src={Trashcan} alt="Delete" width={'30rem'} onClick={() => deleteNoteFromList(note.id)}/>
            </div>
          </li>
        ))}
      </ul>
  ...
  return (
    <div className='absolute w-full h-full bg-gray-200'>
      ...
      {renderNotes()}
      {showAddNewNote && (
        <AddNewNote setShowAddNewNote={setShowAddNewNote} addNewNoteToList={addNewNoteToList} note={{ title: '', content: '' }}/>
      )}
      {currentNote && (
        <Routes>
          <Route
